...
import { Paper, TextField, Button, Grid, Box, Typography } from '@material-ui/core';
...
export default class User extends Component {
	...
	confirmForm = (event) => {
		...
		console.log(this.state.touched.name, this.state.touched.id, this.state.touched.phone, this.state.touched.email, this.state.touched.password);
		// Todos tocados
		if (this.state.touched.name && this.state.touched.id && this.state.touched.phone && this.state.touched.email && this.state.touched.password) {
			...
			UserService.createUser({
				...
				.then((result) => {
					...
					if (result.ok) {
						// Seta mensagem com o result.message
			...
			console.log('Não disparou gravação');
		}
	...
	render() {
		console.log('REDIRECT',this.state.redirectToLogin.redirect);
						...
						<Paper bgcolor="primary.main" >
							<form action="/" onSubmit={this.confirmForm}>
								<Grid container direction="column" alignContent="center" alignItems="center" spacing={1}>
									...
									<Grid item container justify="space-evenly" direction="row">

										<Grid Item >
											<Button variant="contained" style={{ margin: 30 }} color="secondary" type="submit">Confirmar</Button>
										</Grid>
										<Grid Item >
											<Button variant="contained" style={{ margin: 30 }} color="secondary" type="submit">Voltar</Button>
										</Grid>
									</Grid>
								</Grid>
